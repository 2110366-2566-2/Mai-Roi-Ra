...
describe("createAnnouncement", () => {
  ...
  it("should successfully create an announcement when the server responds with 200", async () => {
    fetchMock.mockResponseOnce(
      JSON.stringify({ data: "announcement created" }),
      { status: 200 },
    );
  ...
  describe("createAnnouncement Error Handling", () => {
    it("should handle non-200 HTTP responses correctly", async () => {
      ...
      fetchMock.mockResponseOnce(JSON.stringify({ message: "Unknown error" }), {
        status: 404,
      });
      ...
      ).rejects.toThrow("Failed to create announcement: 404 - Unknown error"); // Match the actual error message
  ...
  describe("createAnnouncement Error Handling", () => {
    it("should handle different error messages when the server responds with non-200 status", async () => {
      ...
      ).rejects.toThrow(`Failed to create announcement: 400 - ${errorMessage}`);
    ...
    it("should handle API errors without an error message", async () => {
      ...
      ).rejects.toThrow("Failed to create announcement: 500 - Unknown error");
