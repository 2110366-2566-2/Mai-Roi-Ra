...
type ModalProps = {
  successReplyModal: boolean;
  setSuccessReplyModal: (value: boolean) => void;
};

export default function SuccessReplyModal({
  successReplyModal,
  setSuccessReplyModal,
}: ModalProps) {
  return (
    <Transition show={successReplyModal} as={Fragment}>
      <Dialog
        as="div"
        className="fixed inset-0 z-50"
        onClose={() => setSuccessReplyModal(false)}
      >
        <Transition.Child
          as={Fragment}
            ...
            >
              ...
              <div className="inline-block align-bottom bg-white rounded-lg text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle max-w-lg w-2/5 h-auto">
                <div className="bg-white p-7 flex justify-center items-center">
                  <div className="text-gray-600">Reply has been sent!</div>
                </div>
              </div>
            </Transition.Child>
          </div>
